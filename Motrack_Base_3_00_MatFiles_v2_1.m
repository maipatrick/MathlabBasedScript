    ...
    function mainmodel(~,~)
        ...
        for n_condition = WC  %Main loop trough the different condition folders
            ...
            for n_sub = WS %Loop troigh  individual subject folders for the given condition
                    ...
                    if isempty(strfind(char(filename(1,i)), OPTIONS.RefSpecifier))
                        ...
                        for ns = 1:numsteps
                            ...
                            for ml = 1 : length (markernames_labels)
                                ...
                                for ml = 1 : length (markernames_labels)
                                    NORMAL.L.MARKERS.(markernames_labels{ml, 1}).X(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Derived.(markernames_labels{ml, 1})(1,ContactL.(structname).data), 0.5)';
                                    NORMAL.L.MARKERS.(markernames_labels{ml, 1}).Y(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Derived.(markernames_labels{ml, 1})(2,ContactL.(structname).data), 0.5)';
                                    NORMAL.L.MARKERS.(markernames_labels{ml, 1}).Z(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Derived.(markernames_labels{ml, 1})(3,ContactL.(structname).data), 0.5)';
                                end
                            end
               ...
               for ml = 1 : length (markernames_labels)
                                NORMAL.L.MARKERS.(markernames_labels{ml, 1}).X(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Opti.(markernames_labels{ml, 1}).data(1,ContactL.(structname).data), 0.5)';
                                NORMAL.L.MARKERS.(markernames_labels{ml, 1}).Y(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Opti.(markernames_labels{ml, 1}).data(2,ContactL.(structname).data), 0.5)';
                                NORMAL.L.MARKERS.(markernames_labels{ml, 1}).Z(:,ns+(runi-1)*numsteps) = normalize(MARKERS.(structname).Opti.(markernames_labels{ml, 1}).data(3,ContactL.(structname).data), 0.5)';
                            end
                            end
